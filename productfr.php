	...
	if(!empty($product_array)){
		...
		?>
			<div class="product content-wrapper">
			 <img src="<?php echo $product_array[$key]['image']?>" width="300" height="300" style="display: block; margin: 0 auto;" alt="<?php echo $product_array[$key]['nom']?>">
				 <div>
					 <h1 class="name" style="text-align: center;"><?php echo $product_array[$key]['nom']?></h1>
					 <label><h3 style="text-align: center;">Prix</label>
					 <span class="price">
					 <?php echo $product_array[$key]['price_eu']."\xE2\x82\xAc"?>
					 </span></h3>
					 
				<div class="description">
					 <?php echo $product_array[$key]['descriere_fr']?>
				</div>
			<form method="POST" action="cos.php?action=add&code=<?php echo $product_array[$key]['code'];?>">
				<div class="cart-action" >
					<input type="text" name="quantity" value="1" size="2"/>
					<input type="submit"  value="Acheter un billet" class="btnAddToCart"/>
				</div>
			</form>
				<div style="text-align: center; margin-bottom: 30px;">
					<a href="films.php"><button>Voir films</button></a>
				</div>
			</div>
		</label>
	</div>
